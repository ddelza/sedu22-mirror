...
- **"스마트 수업도구" 카테고리**: 단원 내용보다 수업에 쓰는 스마트 기기/도구 자체(활용법, 사용 후기, 팁)가 글의 핵심 주제일 때 붙인다. `topic`은 다음 중 하나:
  - `"MBL센서"`, `"아두이노"`, `"앱/프로그램"`, `"생성형 AI 도구"`, `"AR/VR"`, `"교육용 단말기(태블릿/크롬북)"`, `"메이커/3D프린팅"`, `"구글도구"`
  - DALL-E, ChatGPT, Playground, Prompt Hunt처럼 이미지·텍스트를 생성하는 AI 도구는 일반 앱과 구분해서 `"생성형 AI 도구"`로 붙인다(일반 웹앱/설문지 등은 `"앱/프로그램"`).
  - `"구글도구"`(2026-09-10 신설) — 구글 문서(워드 대응)·스프레드시트(엑셀 대응)·슬라이드(프레젠테이션 대응) 등 **구글 워크스페이스 도구를 수업에 활용**하는 게 글의 핵심일 때 붙인다. 예: 학생들과 협업 문서 작성, 스프레드시트로 데이터 공동 기록/그래프 그리기, 슬라이드 공동 제작·발표. 노트북LM/제미나이처럼 AI 기능이 핵심이면 `"생성형 AI 도구"`를 대신(또는 같이) 쓰고, 구글 클래스룸이나 구글 폼처럼 워드/시트/슬라이드가 아닌 다른 구글 도구는 이 topic이 아니라 `"앱/프로그램"`으로 분류한다.
  ```json
