// next-remaining-boards-review-batch.js로 뽑은 배치를 검토한 결과
// (scratch/remaining-boards-review-result.json)를 적용한다. tags가 있는 항목은 기존 "미분류" 판정을
// 새 태그로 교체하고("게시판별 분류" 태그는 보존), tags가 없거나 빈 배열인 항목은 "미분류" 유지로 보고
// 검토완료 상태로만 표시한다.
//
// 입력 형식: [{ "id": "fldid-dataid", "tags": [...태그객체...] | null, "tagsNote": "..." }, ...]
// 사용법: node apply-remaining-boards-review.js [입력경로]
const fs = require('fs');
...
const IN_PATH = process.argv[2] || path.join(__dirname, '..', 'scratch', 'remaining-boards-review-result.json');
