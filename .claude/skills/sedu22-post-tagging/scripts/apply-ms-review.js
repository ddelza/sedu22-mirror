// next-ms-review-batch.js로 뽑은 배치를 검토한 결과(scratch/ms-review-result.json)를
// 적용한다. 기존 태그는 절대 건드리지 않고, addTags에 있는 태그만 이어붙인다.
// (수직/수평 매핑으로 보완할 게 없는 글도 addTags: []로 반드시 포함시켜서
// "검토 완료" 상태로 표시해야 함 — 그래야 다음 배치에서 건너뛴다.)
//
// 입력 형식: [{ "id": "fldid-dataid", "addTags": [...tag객체...], "note": "보완 근거 한줄" }, ...]
// 사용법: node apply-ms-review.js [입력경로]
const fs = require('fs');
...
const IN_PATH = process.argv[2] || path.join(__dirname, '..', 'scratch', 'ms-review-result.json');
