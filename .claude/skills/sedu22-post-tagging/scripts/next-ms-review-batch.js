// 이미 "중학교" 실단원으로 태깅된 게시글을 처음부터 순서대로 다시 훑으면서,
// 고등학교 태그(수직 매핑)나 2015/2022 교차 태그(수평 매핑)를 추가로 붙일 수
// 있는지 검토하기 위한 배치 추출 스크립트.
//
// next-batch.js와 달리 "미태깅"이 아니라 "이미 중학교로 태깅된 글"을 대상으로 하며,
// 진행 상태를 scratch/ms-review-state.json에 "이미 검토한 id 목록"으로 저장해
// 재실행해도 처음부터 다시 훑지 않도록 한다.
//
// 사용법: node next-ms-review-batch.js [배치크기]
const fs = require('fs');
const path = require('path');

const REPO = path.join(__dirname, '..', '..', '..', '..');
const POSTS_DIR = path.join(REPO, 'data', 'posts');
const STATE_PATH = path.join(__dirname, '..', 'scratch', 'ms-review-state.json');
const OUT_PATH = path.join(__dirname, '..', 'scratch', 'ms-review-batch.json');
...
function isMsTagged(post) {
  return !!(post.tags && post.tags.some(t => t.schoolLevel === '중학교' && t.unit && t.unit !== '미분류'));
}
...
console.log(`중학교 태깅된 전체: ${totalMsTagged}건 / 아직 검토 안 한: ${totalRemaining}건`);
